...
Route::prefix('dashboard')->as('admin.')->group(function () {
    ...
    Route::middleware(['auth', 'verified', 'role:admin'])->group(function () {
        ...
        Route::put('/my-notes/{note:id}/update', [NoteController::class, 'updateAndBack'])->name('note.updateAndBack');
        Route::delete('/my-notes/{note:id}', [NoteController::class, 'destroy'])->name('note.destroy');
        Route::put('/my-notes/{note:id}/unpin', [NoteController::class, 'unpin'])->name('note.unpin');
        Route::put('/my-notes/{note:id}/pin', [NoteController::class, 'pin'])->name('note.pin');
...
Route::get('/notes/{note}/{slug}', [NoteController::class, 'publicShow'])->name('note.show');
Route::post('/notes/{note}/password', [NoteController::class, 'checkPassword'])->name('note.checkPassword');
Route::get('/notes/{note}/{slug}/edit', [NoteController::class, 'publicEdit'])->middleware('auth', 'verified', 'role:admin')->name('note.publicEdit');
Route::get('/s/notes/{note:sharable_link}', [NoteController::class, 'showShared'])->name('note.sharedShow');
...
Route::post('/contact/send', [ContactController::class, 'send'])->middleware('throttle:5,1,contact')->name('contact.send');
